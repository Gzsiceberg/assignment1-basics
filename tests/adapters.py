...
def run_swiglu(
    ...
    swiglu = SwiGLU(d_model, dim_ff=d_ff)
...
def run_multihead_self_attention_with_rope(
    ...
    multi_head_attention = MultiHeadAttention(d_model, num_heads, rope=rope)
    multi_head_attention.wq.weight.data = q_proj_weight
    multi_head_attention.wk.weight.data = k_proj_weight
    multi_head_attention.wv.weight.data = v_proj_weight
    multi_head_attention.wo.weight.data = o_proj_weight
...
def run_silu(in_features: Float[Tensor, " ..."]) -> Float[Tensor, " ..."]:
    ...
    """
    from cs336_basics.model.activation import silu_activation
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    return softmax_activation(in_features, dim)
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    from cs336_basics.model.optimizer import gradient_clipping
...
def run_get_lr_cosine_schedule(
    ...
    """
    from cs336_basics.model.optimizer import get_lr_cosine_schedule
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    from cs336_basics.train_bpe import train_bpe, BPETokenizerParams
    input_path_str: str = os.fspath(input_path)
    bpe_params: BPETokenizerParams = train_bpe(input_path_str, vocab_size, special_tokens)
    return bpe_params.vocab, bpe_params.merges_list
